...
if args.trainset == 'CIFAR10':
    CalcuSSIM = MS_SSIM(window_size=3, data_range=1., levels=4, channel=3).to("cuda")
else:
    CalcuSSIM = MS_SSIM(data_range=1., levels=4, channel=3).to("cuda")

def load_weights(model_path):
    pretrained = torch.load(model_path, map_location=torch.device('cuda'))
